...
def solve_implicit_scheme(K, M, F, nodes, elements, dt, num_steps, V0, omega):
    num_nodes = K.shape[0]
...
frequencies = np.array([1e6,1e9,1e4,0.01])  # Frequencies
real_flux = []
imag_flux = []
...
for omega in frequencies:
    ...
    phi = solve_implicit_scheme(K, M, F, nodes, elements, dt, num_steps, V0, omega)
    print(phi)
    # Calculate current
